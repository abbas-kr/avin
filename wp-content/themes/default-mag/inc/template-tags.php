	...
	function default_mag_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links twp-links"><span class="twp-icon"><i class="fal fa-calendar"></i></span>' . esc_html__( 'Posted in %1$s', 'default-mag' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links twp-links"><span class="twp-icon"><i class="fal fa-tags"></i></span>' . esc_html__( 'Tagged %1$s', 'default-mag' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		...
		edit_post_link(
			...
			'<span class="edit-link twp-links"><span class="twp-icon"><i class="fal fa-edit"></i></span>',
			'</span>'
		);
	}
endif;
        	    ...
        	    <i class="fal fa-clock-o"></i>
        	        ...
        	        if ($site_date_layout_option == 'normal-format') {
        	           the_time(get_option('date_format'));
        	            ...
        	            echo human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ' . __('قبل', 'default-mag');
        	        }
